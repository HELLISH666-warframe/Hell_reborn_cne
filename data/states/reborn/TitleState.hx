...
function create() {
	...
	logo = new FlxSprite();
	logo.frames = Paths.getSparrowAtlas('menus/titlescreen/logoBumpin');
	...
	textArray = Assets.getText('data/startText.txt', false).split('|-|-|-|');
...
function update(elapsed:Float) {
	sheddar.data.iTime.value=[Sys.cpuTime() * elapsed];
...
public function onEndIntro() {
	...
	if (FlxG.sound.music.time < 24850) FlxG.sound.music.time = 24850;
...
class TTest extends FlxSprite {
	...
	function new(X:Float = 0, Y:Float = 0, minWidth:Int = 35, maxWidth:Int = 150, _height:Int = 50, _color:FlxColor = 0xFFffffff, duration:Float = 1,
			?goal:Float) {
		super(X, Y);
