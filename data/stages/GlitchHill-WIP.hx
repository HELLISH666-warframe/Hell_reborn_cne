...
function stepHit(curStep){
    ...
    if (curStep == 1041)
    {
        boyfriend.scale.set(0.5, 0.5);
        dad.scale.set(0.5, 0.5);
        var rainbTmr = new FlxTimer().start((Conductor.songPosition / 800000000), function(tmr:FlxTimer)
            {
                background.x -= (Math.sin(Conductor.songPosition)/99999)+40;
                tmr.reset(Conductor.songPosition / 90000000);}
            );
    }
}