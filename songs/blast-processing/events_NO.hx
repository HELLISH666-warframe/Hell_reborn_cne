...
function GO_UP() {
    trace('UP');
    var UPtimer = new FlxTimer().start(0.01, function(tmr:FlxTimer)
        {
            FlxTween.tween(dad, {y: 600}, 0.01,{ 
...
function DOWNNN2() {
    trace('DOWN2');
    var DOWNtimer = new FlxTimer().start(0.001, function(tmr:FlxTimer)
        {
            FlxTween.tween(dad, {y: 1000}, 0.5,{ 
                ease: FlxEase.linear, onComplete: function(twn:FlxTween) 
                    {STRAIAIGHT1();}});
           // dad.y += (Math.sin(time)/2)+200;
          //  GO_UP();}
...
function GO_UP1() {
    trace('UP2');
    var UPtimer = new FlxTimer().start(0.3, function(tmr:FlxTimer)
        {
